...
{
    public partial class ListQtlFrom : Form
    {
        ...
        private void ListQtlFrom_Load(object sender, EventArgs e)
        {

            laDwsjValue.Text = db.GetSingleObject("SELECT gridmanager.secretary FROM gridmanager WHERE gridmanager.id = '2'").ToString();
            laJwhzrValue.Text = db.GetSingleObject("SELECT gridmanager.director FROM gridmanager WHERE gridmanager.id = '2'").ToString();
            laDwsjTelValue.Text = db.GetSingleObject("SELECT gridmanager.secretary_code FROM gridmanager WHERE gridmanager.id = '2' ").ToString();
            laJwhzrTelValue.Text = db.GetSingleObject("SELECT gridmanager.director_code FROM gridmanager WHERE gridmanager.id = '2' ").ToString();
            laDwsjPhoneValue.Text = db.GetSingleObject("SELECT gridmanager.secretary_phone FROM gridmanager WHERE gridmanager.id = '2' ").ToString();
            laJwhzrPhoneValue.Text = db.GetSingleObject("SELECT gridmanager.director_phone FROM gridmanager WHERE gridmanager.id = '2' ").ToString();

            laYardValue.Text = db.GetSingleObject("SELECT Count(population.id) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN population ON population.block = block.id WHERE district.id = '2'").ToString();
            laFamilyValue.Text = db.GetSingleObject("SELECT Count(population.id) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN population ON population.block = block.id INNER JOIN features ON population.features = features.id INNER JOIN resident ON features.resident = resident.id INNER JOIN residentaddresss ON resident.resident_addresss = residentaddresss.id WHERE residentaddresss.`host` = '1' AND district.id = 2 ").ToString();
            laCommunityPopulationValue.Text = db.GetSingleObject("SELECT Count(population.`name`) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN population ON population.block = block.id WHERE district.id = '2'").ToString();
            ...
            laCommunistValue.Text = db.GetSingleObject("SELECT Count(population.educational) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN population ON population.block = block.id WHERE district.id = '2' AND population.educational = '党员'").ToString();
            laOlderValue.Text = db.GetSingleObject("SELECT Count(features.old) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN population ON population.block = block.id INNER JOIN features ON population.features = features.id WHERE district.id = '2' ").ToString();
            laAloneOlderValue.Text = db.GetSingleObject("SELECT Count(features.old_alone) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN population ON population.block = block.id INNER JOIN features ON population.features = features.id WHERE district.id = '2' AND features.old_alone = '1'").ToString();
